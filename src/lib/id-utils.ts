...
export function generateMessageId(prefix: string = 'msg', includeRandomSeed: boolean = false): string {
  ...
  while (attempts < MAX_RETRY_ATTEMPTS) {
    try {
      ...
      if (error) {
        console.warn(`Error incrementing message counter (attempt ${attempts + 1}), using fallback`);
...
export function generateSecureId(
  ...
  while (attempts < maxRetries) {
    try {
      ...
      const { data: secureResult, error: cryptoError } = safeSync(
        () => {
          ...
          if (typeof window !== 'undefined' && window.location && window.location.protocol !== 'https:' && window.location.hostname !== 'localhost') {
            console.warn('Crypto API may not work reliably over HTTP');
          }
      ...
      console.warn(`Crypto API not available (attempt ${attempts + 1}), using Math.random fallback`);
...
export function generateUUID(): string {
  try {
    ...
    console.warn('Crypto APIs not available, using Math.random UUID fallback');
...
export function generateIdBatch(
  ...
  try {
    ...
    if (ids.size < count) {
      console.warn(`Could only generate ${ids.size} unique IDs out of requested ${count}`);
    }
