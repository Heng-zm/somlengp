/*
  ...
  - name: English country/territory name
  - iso2: ISO 3166-1 alpha-2 code (uppercase)
  - dialCode: E.164 country calling code digits without '+' (NANP area-based territories use full code e.g. 1264)

  Helpers:
  - flagEmoji: render regional indicator flag from iso2
  - findByIso2, findAllByDialCode, bestGuessByPhone
*/

export type Country = {
  name: string;
  iso2: string; // e.g. "US"
  dialCode: string; // e.g. "1", "91", "1264"
};
...
export const countries: Country[] = [
  ...
  { name: "Korea (North)", iso2: "KP", dialCode: "850" },
  { name: "Korea (South)", iso2: "KR", dialCode: "82" },
...
const byDial = new Map<string, Country[]>(
  Object.entries(
    countries.reduce<Record<string, Country[]>>((acc, c) => {
      if (!acc[c.dialCode]) acc[c.dialCode] = [];
      acc[c.dialCode].push(c);
      return acc;
    }, {})
  )
);
...
export function flagEmoji(iso2: string): string {
  ...
  return String.fromCodePoint(...code.split("").map(ch => A + (ch.charCodeAt(0) - 65)));
}
...
export function findAllByDialCode(dialCode: string): Country[] {
  const dc = dialCode.replace(/^\+/, "");
  return byDial.get(dc) ?? [];
}

export function bestGuessByPhone(input: string): Country[] {
  // Returns countries whose dial code matches the longest prefix of the digits in input
  const digits = (input || "").replace(/\D+/g, "");
  if (!digits) return [];
  let matches: Country[] = [];
  for (let i = Math.min(4, digits.length); i >= 1; i--) {
    const prefix = digits.slice(0, i);
    const found = byDial.get(prefix);
