...
function toast({ ...props }: Toast) {
  const { data: toastResult, error } = safeSync(
    () => {
      ...
      if (props.title && typeof props.title === 'string' && props.title.length > MAX_TOAST_TITLE_LENGTH) {
        console.warn(`Toast title truncated from ${props.title.length} to ${MAX_TOAST_TITLE_LENGTH} characters`)
      ...
      if (props.description && typeof props.description === 'string' && props.description.length > MAX_TOAST_DESCRIPTION_LENGTH) {
        console.warn(`Toast description truncated from ${props.description.length} to ${MAX_TOAST_DESCRIPTION_LENGTH} characters`)
  ...
  if (error) {
    ...
    return {
      id: fallbackId,
      dismiss: () => { console.warn('Fallback dismiss called'); },
      update: () => { console.warn('Fallback update called'); },
    }
  }
  return toastResult || {
    id: 'error',
